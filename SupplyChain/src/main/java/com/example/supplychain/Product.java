...
public class Product {
    ...
    Product (int id , String name , double price){
     ...
     public static ObservableList<Product>getAllproducts(){
        ...
        String selectAllProducts = "SELECT * FROM supply_chain.product";
        try {
    ...
    public static ObservableList<Product>getProductByName(String name){
        DatabaseConnection dbConn = new DatabaseConnection();
        ObservableList<Product> data = FXCollections.observableArrayList();
        String selectAllProducts = String.format("SELECT * FROM supply_chain.product WHERE LOWER(name) like '%%%s%%'",name.toLowerCase());
        try {
            ResultSet rs = dbConn.getQueryTable(selectAllProducts);
            while (rs.next()){
                data.add(new Product(rs.getInt("pid"),
